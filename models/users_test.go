...
func testUserToManyPosterComments(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.PosterComments().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.PosterID == b.PosterID {
			bFound = true
		}
		if v.PosterID == c.PosterID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPosterComments(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PosterComments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PosterComments = nil
	if err = a.L.LoadPosterComments(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PosterComments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpPosterComments(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddPosterComments(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.PosterID {
			t.Error("foreign key was wrong value", a.ID, first.PosterID)
		}
		if a.ID != second.PosterID {
			t.Error("foreign key was wrong value", a.ID, second.PosterID)
		}

		if first.R.Poster != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Poster != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.PosterComments[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PosterComments[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PosterComments().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `INTEGER`, `Username`: `VARCHAR(30)`, `Password`: `VARCHAR(200)`, `Salt`: `VARCHAR(100)`, `Role`: `VARCHAR(15)`}
	_           = bytes.MinRead
)
