...
type userR struct {
	PosterComments CommentSlice `boil:"PosterComments" json:"PosterComments" toml:"PosterComments" yaml:"PosterComments"`
}
...
var (
	userAllColumns            = []string{"id", "username", "password", "salt", "role"}
	userColumnsWithoutDefault = []string{"username", "password", "salt", "role"}
...
// PosterComments retrieves all the comment's Comments with an executor via poster_id column.
func (o *User) PosterComments(mods ...qm.QueryMod) commentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"comments\".\"poster_id\"=?", o.ID),
	)
...
// LoadPosterComments allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPosterComments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`comments`),
		qm.WhereIn(`comments.poster_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.PosterComments = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.PosterID {
				local.R.PosterComments = append(local.R.PosterComments, foreign)
				if foreign.R == nil {
					foreign.R = &commentR{}
				}
...
// AddPosterComments adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.PosterComments.
// Sets related.R.Poster appropriately.
func (o *User) AddPosterComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			PosterComments: related,
		}
	} else {
		o.R.PosterComments = append(o.R.PosterComments, related...)
	}
