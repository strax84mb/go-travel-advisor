...
func testCommentToOneUserUsingPoster(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.PosterID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Poster().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadPoster(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Poster == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Poster = nil
	if err = local.L.LoadPoster(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Poster == nil {
		t.Error("struct should have been eager loaded")
...
func testCommentToOneCityUsingCity(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CommentSlice{&local}
	if err = local.L.LoadCity(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.City == nil {
		t.Error("struct should have been eager loaded")
...
func testCommentToOneSetOpUserUsingPoster(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		...
		if x.R.PosterComments[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.PosterID != x.ID {
			t.Error("foreign key was wrong value", a.PosterID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.PosterID))
		reflect.Indirect(reflect.ValueOf(&a.PosterID)).Set(zero)
		...
		if a.PosterID != x.ID {
			t.Error("foreign key was wrong value", a.PosterID, x.ID)
		}
	}
}
func testCommentToOneSetOpCityUsingCity(t *testing.T) {
	var err error
...
var (
	commentDBTypes = map[string]string{`ID`: `INTEGER`, `CityID`: `BIGINT`, `PosterID`: `BIGINT`, `Text`: `VARCHAR(255)`, `Created`: `DATETIME`, `Modified`: `DATETIME`}
	_              = bytes.MinRead
)
