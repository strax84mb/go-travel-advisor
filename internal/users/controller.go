...
func (uc *userController) Login(w http.ResponseWriter, r *http.Request) {
	...
	err := handler.GetBodyFF(r, &userLogin)
	if err != nil {
	...
	handler.RespondFF(w, http.StatusOK, &payload)
}
...
func (uc *userController) GetUserByUsername(w http.ResponseWriter, r *http.Request) {
	...
	handler.RespondFF(w, http.StatusOK, dto.UserToDto(user))
}
...
func (uc *userController) GetUserById(w http.ResponseWriter, r *http.Request) {
	...
	user, err := uc.userSrvc.GetByID(r.Context(), id)
	if err != nil {
		handler.ResolveErrorResponse(w, err)
		return
	}
