...
func NewCommentController(commentSrvc iCommentService, r *handler.Responder) *commentController {
...
func (cc *commentController) listComments(w http.ResponseWriter, r *http.Request) {
	...
	cc.r.Respond(w, http.StatusOK, dto.CommentsToDtos(comments))
}
...
func (cc *commentController) getCommentByID(w http.ResponseWriter, r *http.Request) {
	...
	cc.r.Respond(w, http.StatusOK, dto.CommentToDto(*comment))
}
...
func (cc *commentController) listCommentsForMe(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		cc.r.ResolveErrorResponse(w, handler.NewErrUnauthorizedWithCause("user not logged in"))
		return
	}
...
func (cc *commentController) listCommentsForUser(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || !utils.HasRole(roles, "admin") {
		cc.r.ResolveErrorResponse(w, handler.NewErrUnauthorizedWithCause("only admins allowed"))
		return
	}
...
func (cc *commentController) saveNewComment(w http.ResponseWriter, r *http.Request) {
	...
	userID, _, ok := utils.GetJWTData(ctx)
	if !ok {
		cc.r.ResolveErrorResponse(w, handler.NewErrUnauthorizedWithCause("must be logged in"))
		return
	}
	...
	cc.r.Respond(w, http.StatusCreated, dto.CommentToDto(*comment))
}
